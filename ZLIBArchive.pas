...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, zlib, FileCtrl;
...
type
  ...
  public
    ...
    procedure AddFilesMask(path:string);
...
procedure TZLBArchive.AddFilesMask(path:string);
     ...
     begin
          ...
          begin
               if (data.name <> '.') and
                  (data.name <> '..') then
               begin
                    if fRecursePaths then ScanPath(extractFilePath(Path)+data.name);
                    if fSavePaths then FoundFiles.add(extractFilePath(Path)+data.name);
               end;
          end
          else
          FoundFiles.add(extractFilePath(Path)+data.name);
          ds := findnext(data);
     end;
     findClose(data);
     AddFiles(FoundFiles);
     ...
     try
        ...
        try
           ...
           try
              ...
              FileInfo^.name := AnsiString(extractfilename(filename));
              FileInfo^.path := AnsiString(extractfilepath(filename));
              if SavePaths then
             ...
             Try
                ...
                try
                   ...
                   try
                      ...
                      fillchar(FileInfo^,sizeof(TZLBFileRec),0);
                      FileInfo^.CRC := GetCRC32(InFile,0);
                      FileInfo^.name := AnsiString(extractfilename(filelist[0]));
                      FileInfo^.path := AnsiString(extractfilepath(filelist[0]));
                      if SavePaths then
                         delete(FileInfo^.path,1,pos(':',FileInfo^.path))
                      else
                         fillchar(FileInfo^.path,sizeof(FileInfo^.path),#0);
...
begin
     ...
     begin
          ...
          try
             ...
             FileInfo.name := AnsiString(extractfilename(new));
             FileInfo.path := AnsiString(extractfilepath(new));
